...
GraphicsDisplay::GraphicsDisplay(Xwindow &w, Game &g) : w{w}, g{g}
{
    ...
    w.drawString(10, 40, "Downloaded: " + std::to_string(currentPlayer1->getNumData()) + "D, " + std::to_string(currentPlayer1->getNumVirus()) + "V");
    ...
    w.drawString(10, 560, "Downloaded: " + std::to_string(currentPlayer2->getNumData()) + "D, " + std::to_string(currentPlayer2->getNumVirus()) + "V");
...
void GraphicsDisplay::notify(Cell &c)
{
    ...
    else if ((sym >= 'a' && sym <= 'h' && g.checkWhoseTurn()) || (sym >= 'A' && sym <= 'H' && !g.checkWhoseTurn()))
    {
        if (linkType == "D")
        {
    ...
    {
        if (!currentPlayer1->getPureLink(sym).checkIfRevealed())
        {
            w.fillRectangle(x, y, cellSize, cellSize, Xwindow::Black);
            w.drawString(x + cellSize / 2, y + cellSize / 2, msg, Xwindow::White);
        }
